...
- **Date shifting (self-healing — both Tableau Next and Pulse)**: all demos use the same self-healing formula that shifts dates relative to the build date and `TODAY()`. The formula logic is identical across platforms — only the syntax differs:
  ...
  - **Pulse**: publish the `.hyper` directly (NOT a `.tdsx`). The Pulse metric's `time_dimension` references the raw `{"field": "Date"}` column. Pulse's internal engine handles time-relative display natively — no calculated field needed on the datasource side. Publishing `.hyper` directly allows Pulse to index the datasource in seconds; publishing `.tdsx` causes indexing delays of 2+ hours (see Known Pitfalls).
  ...
  - **Result**: the most recent data point always appears as today, the entire timeline floats forward automatically, no manual refresh or re-publish needed.
...
- Pulse: ALWAYS publish `.hyper` directly for Pulse datasources — NEVER publish `.tdsx`. Pulse indexes `.hyper` files in seconds but takes 2+ hours (or indefinitely) to index `.tdsx` packages. Publishing `.tdsx` causes metric creation to fail with 404 "Not Found" because the datasource isn't indexed yet. The `time_dimension` in metric definitions should reference the raw `Date` column.
