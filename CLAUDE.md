...
- **Date shifting (self-healing — both Tableau Next and Pulse)**: all demos use the same self-healing formula that shifts dates relative to the build date and `TODAY()`. The formula logic is identical across platforms — only the syntax differs:
  ...
  - **Pulse (.tdsx calculated field)**: `DATEADD('day', DATEDIFF('day', #<build_date>#, [Date]), TODAY())` — uses single quotes, bare field name from datasource. Embed in a `.tds` XML inside a `.tdsx` archive. The Pulse metric's `time_dimension` references `{"field": "Display Date"}`. Tableau Cloud evaluates `TODAY()` at query time (not at extract refresh) because "unstable functions" are excluded from materialization.
...
- **Pulse .tdsx packaging**: publish a `.tdsx` (ZIP archive) instead of a raw `.hyper`. Structure: `{slug}.tds` (XML with connection + calculated field) + `Data/Extracts/{slug}.hyper` (physical data). The `.tds` defines the `Display Date` calculated field. TSC publishes `.tdsx` files identically to `.hyper` — `server.datasources.publish(ds_item, tdsx_path, "Overwrite")`.
