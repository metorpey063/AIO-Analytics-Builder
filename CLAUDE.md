...
- Pulse: Definition creation response nests the ID under `metadata.id` — NOT at the top level or under `definition.id`. Parse with: `def_id = r.json().get("metadata", {}).get("id")`. Getting this wrong means the subsequent `GET /definitions/{def_id}/metrics` call fails, which breaks subscription creation.
