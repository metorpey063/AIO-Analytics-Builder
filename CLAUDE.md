...
- **Date shifting (Tableau Next)**: after creating the SDM, create a calculated dimension named `Display Date` using `DATEADD("day", 0, [DO_apiName].[date_field_apiName])` and save `display_date_api`, `date_field_api`, and `build_date` (= `date.today().isoformat()`) to the checkpoint. Point all metric `timeDimensionReference` fields at the Display Date dimension instead of the raw date. The `/refresh-demo` skill updates this offset so the signal stays current without re-ingesting data. Calculated dimensions live at `POST /services/data/v65.0/ssot/semantic/models/{sdm}/calculated-dimensions` with payload `{apiName, label, expression, dataType: "Date"}` and can be updated with `PUT` to the same path. `TODAY()` and `DATEADD` work in expressions; composing them with `MAX()` does not (row/aggregate level mismatch).
