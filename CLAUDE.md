...
- Pulse: **2026.2 API version mismatch** — `POST /api/-/pulse/definitions` returns 400 "Invalid request" on Tableau Cloud pods that have received the 2026.2 release but NOT the 262.1 Hotfix. The metric definition format changed in 2026.2 and is not backward compatible. Symptoms: all POST payloads fail (even empty body or known-working datasources), but GET, PATCH (name/extension_options), and DELETE work fine. Build is `2026.2.1` (`20262.26.0609.1953`). **Workaround**: wait for the 262.1 Hotfix to roll out to your pod, or use a site on a hotfixed pod (UWC or 10AX). Check your pod's build via `GET /api/{version}/serverinfo` → `productVersion.build`. The hotfix was expected mid-June 2026. Once patched, re-run the demo script — checkpoint logic will skip data gen and resume at metric creation.
