...
- **Date shifting (self-healing — both Tableau Next and Pulse)**: all demos use the same self-healing formula that shifts dates relative to the build date and `TODAY()`. The formula logic is identical across platforms — only the syntax differs:
  ...
  - **Result — Pulse**: dates are correct at build time. Run `/refresh-demo` before a meeting if the demo is more than a week old (~30 seconds). `use_dynamic_offset` ensures Pulse never shows empty/broken data even without a refresh.
...
- Pulse: ALWAYS publish `.hyper` directly for Pulse datasources — NEVER publish `.tdsx`. Pulse indexes `.hyper` files in seconds but NEVER indexes `.tdsx` packages on Tableau Cloud (tested: 14+ hours with no indexing). Publishing `.tdsx` causes metric creation to fail with 404 "Not Found". The `time_dimension` must reference the raw `Date` column. Calculated fields (like `Display Date`) are not available to Pulse — use `/refresh-demo` to keep dates current instead.
- Pulse: `extension_options` (including `use_dynamic_offset`, `allowed_dimensions`, `allowed_granularities`) are NOT persisted when included in the POST create payload on Tableau Cloud 26.2+. They must be set via a separate PATCH call: `PATCH /api/-/pulse/definitions/{id}` with Content-Type `application/vnd.tableau.metricqueryservice.v1.UpdateDefinitionRequest+json`. The PATCH response confirms the values but subsequent GET may return empty `{}` — this is a display bug; the settings are stored internally.
...
- Visualizations field role exclusivity: a field must be EITHER a dimension (grouping, placed in `columns`) OR a measure (aggregation, placed in `rows`) — never both in the same visualization. Assigning the same `fieldName` to both a grouping slot and an aggregation slot causes the chart to fail to render silently. Each semantic field should appear in exactly one role per viz.
